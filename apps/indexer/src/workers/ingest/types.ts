...
export type Context<C = unknown> = {
  chain: Chain;
  checkpoint: TIngestionSource;
} & C;

export type SyncBatch<T = unknown> = { items: T[]; nextCursor?: string };

export type BackfillResult<T = unknown> = SyncBatch<T> & {
  atLiveEdge?: boolean;
};
